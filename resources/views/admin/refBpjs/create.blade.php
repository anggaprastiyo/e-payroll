                    ...
                    <form method="POST" action="{{ route("admin.ref-bpjs.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('provider') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\RefBpj::PROVIDER_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="provider_{{ $key }}" name="provider" value="{{ $key }}" {{ old('provider', '') === (string) $key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('jenis_beban') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\RefBpj::JENIS_BEBAN_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="jenis_beban_{{ $key }}" name="jenis_beban" value="{{ $key }}" {{ old('jenis_beban', '') === (string) $key ? 'checked' : '' }} required>
