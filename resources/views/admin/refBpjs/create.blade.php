...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.refBpj.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ref-bpjs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\RefBpj::PROVIDER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('provider') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="provider_{{ $key }}" name="provider" value="{{ $key }}" {{ old('provider', '') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('presentase') ? 'is-invalid' : '' }}" type="number" name="presentase" id="presentase" value="{{ old('presentase', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\RefBpj::JENIS_BEBAN_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('jenis_beban') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="jenis_beban_{{ $key }}" name="jenis_beban" value="{{ $key }}" {{ old('jenis_beban', '') === (string) $key ? 'checked' : '' }} required>
