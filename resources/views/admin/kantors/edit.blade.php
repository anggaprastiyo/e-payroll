    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.kantors.update", [$kantor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $kantor->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('alamat') ? 'is-invalid' : '' }}" name="alamat" id="alamat">{{ old('alamat', $kantor->alamat) }}</textarea>
